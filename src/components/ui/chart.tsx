...
>(({ active, payload = [], className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey }, ref) => {
	...
	const tooltipLabel = React.useMemo(() => {
		...
		return value ? <div className={cn("font-medium", labelClassName)}>{labelFormatter ? labelFormatter(value, payload) : value}</div> : null;
	...
	return (
		<div ref={ref} className={cn("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-[color:var(--border)]/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
