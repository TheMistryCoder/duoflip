...
const solutions = [
  {
    ...
    features: [
      "Sensory stimulation through interactive visuals",
      "Encourages social interaction and communication", 
      "Adaptable content for different needs and abilities"
    ],
  ...
  },
  {
    ...
    features: [
      "Kinesthetic learning for better retention",
...
export default function Solutions() {
  ...
  return (
    <section id="solutions" className="py-20 bg-white" ref={ref}>
      <div className="site-container">
        ...
        >
          ...
          <h2 className="text-slate-800 mb-6">Proven Applications Across Educational Settings</h2>
          <p className="section-subtitle max-w-4xl mx-auto">
            Discover how leading schools implement Duo Flip to create engaging, inclusive learning environments that support diverse educational needs
          </p>
        </motion.div>
        ...
        {solutions.map((solution, index) => (
          <motion.div
            key={index}
            className={`grid md:grid-cols-2 gap-12 items-center mb-16 ${index === 1 ? 'md:direction-reverse' : ''}`}
            initial={{ opacity: 0, y: 30 }}
            animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
          ...
          >
            <div className={`space-y-6 ${index === 1 ? 'md:order-2' : ''}`}>
              <div className="flex items-center space-x-4">
                <motion.div 
                  className={`w-12 h-12 bg-gradient-to-br ${solution.gradient} rounded-full flex items-center justify-center`}
              ...
              <ul className="space-y-2 text-slate-600">
                {solution.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-center space-x-3">
                    <div className={`w-2 h-2 ${solution.dotColor} rounded-full`}></div>
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
            ...
            <motion.div 
              className={`relative ${index === 1 ? 'md:order-1' : ''}`}
              whileHover={{ scale: 1.02 }}
              transition={{ duration: 0.3 }}
            >
              <img 
                src={solution.image} 
                alt={`${solution.title} classroom environment`}
